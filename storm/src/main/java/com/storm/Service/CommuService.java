...
//총 데이터 개수 구하기 질의 명령 실행 함수
	...
	public ArrayList getBlist(int nowPage, PageUtil pInfo, CommuVO CVO) {
		...
		System.out.println("$$$$$$$$ 여기는 ?서비스getblist"+list.toString());
 ...
 */
	...
	public int getUserkey(String id) {
		return CDAO.getUserkey(id);
	}
	
	//댓글을 디비에 입력
	public void insertReply(int usrKey) {
		CDAO.insertReply(usrKey);
		
	}
	
	//	원글의 정보를 알아내는 서비스 함수
	public CommuVO getOriInfo(int oriNo) {
		return CDAO.getOriInfo(oriNo);
	}
	
	
	//	원글의 정보를 알아내는 서비스 함수
	public CommuVO getReply(int oriNo) {
		return CDAO.getReply(oriNo);
	}

	// 커뮤니티의 하나의 게시 글 하나를 반환하는 메서드
	public CommuVO getBoard(int boardno) {
		return CDAO.getBoard(boardno);
	}
}
