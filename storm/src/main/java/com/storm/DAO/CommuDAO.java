...
*/
	...
	public void binsert(CommuVO CVO) {
		System.out.println("##### DAO insert VO.communo : " + CVO.boardtext);
 ...
 */
	...
	public int getUserkey(String id) {
		System.out.println("dao: getUserkey "+id);
		CommuVO CVO= sSession.selectOne("commu.getUserkey", id);
		return CVO.usrKey;
	}
	
	//댓글디비입력
	public void insertReply(int usrKey) {
		sSession.insert("commu.insertReply", usrKey);
		
	}
	
	//원글정보 불러오기
	public CommuVO getOriInfo(int oriNo) {
		CommuVO CVO = (CommuVO)sSession.selectList("commu.getOriNo", oriNo);
		System.out.println("dao : getOriInfo : "+oriNo);
		return CVO;
	}
	public CommuVO getBoard(int boardno) {
		return sSession.selectOne("commu.boardView", boardno);
	}
	public CommuVO getReply(int oriNo) {
		CommuVO CVO = (CommuVO)sSession.selectList("commu.getOriNo", oriNo);
		System.out.println("dao : getOriInfo : "+oriNo);
		return CVO;
	}
